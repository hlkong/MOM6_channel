...
public channel8_initialize_topography
...
! -----------------------------------------------------------------------------
!> This subroutine sets up the channel8 test case topography.
!> channel8 is similar to channel6 but:
!> 1) Scotia Arc is removed
!> 2) A ridge is put across Drake Passage, 2500m deep

subroutine channel8_initialize_topography(D, G, param_file, max_depth)
  ...
  character(len=40)  :: mod = "channel8_initialize_topography" ! This subroutine's name.
  ...
  call MOM_mesg("  channel8_initialization.F90, channel8_initialize_topography: setting topography", 5)
  ...
  do i = is,ie                ! zonal grid points
    ...
    D(i,j) = 1.0 - spike(x-dx/2, ll/lonext)*spike(min(0.0, y-reentrantn-sdp/2.0), sdp) &           ! Patagonia, west
                ...
                - sa * homo(x-1.0/lonext-dx/2, 1.0/lonext) * homo(y-8.0/latext, 3.0/latext)        ! Ridge across Drake Passage
...
end subroutine channel8_initialize_topography
